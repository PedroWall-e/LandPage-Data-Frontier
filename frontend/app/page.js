...
export default function HomePage() {
    ...
    if (newPassword.length === 4) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setTimeout(() => setShowKeypad(false), 3000);
        } else {
  ...
  const handleRecoverPassword = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setKeypadStatus({ type: 'success', message: 'Instrução enviada para o email!' });
      } else {
  ...
  return (
    <div
      className="min-h-screen font-sans pb-16 selection:bg-[#3347FF] selection:text-white"
      style={{ backgroundColor: colors.lightBg, color: colors.dark }}
    >
      <main className="w-full max-w-5xl mx-auto px-4 py-8 md:py-12 flex flex-col gap-6">
        ...
        {/* 2. LINHA DE SERVIÇOS 1: Academy e IoT */}
        ...
        {/* 3. LINHA DE SERVIÇOS 2: STL e Resinas */}
        <div className="flex flex-col md:flex-row gap-6">
        ...
        {/* 4. LINHA DE SERVIÇOS 3: Usinagem e Robótica */}
        <div className="flex flex-col md:flex-row gap-6">
        ...
        <ContentBlock bgColor={colors.white} className="border-t-4" style={{ borderTopColor: colors.blue }}>
          ...
          <form className="max-w-3xl mx-auto w-full flex flex-col gap-5" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full mt-4 flex items-center justify-center gap-3 px-8 py-5 rounded-2xl font-bold text-white text-lg hover:opacity-90 hover:-translate-y-1 transition-all active:scale-95 shadow-lg ${
      ...
      <footer className="w-full text-center py-10 text-sm text-gray-500 font-medium flex flex-col gap-2">
        ...
        <p>
          Telefone:{' '}
          <a href="tel:+5531975280637" className="hover:text-[#3347FF] transition-colors">
            +55 31 97528-0637
          </a>
        </p>
      </footer>
              ...
              {[...Array(4)].map((_, i) => (
                <div
                  ...
                  className={`w-4 h-4 rounded-full transition-colors ${
                    i < keypadPassword.length ? 'bg-[#3347FF]' : 'bg-gray-200'
                  }`}
                />
              ))}
            </div>
            ...
            {keypadStatus.message && (
              <div
                className={`p-3 rounded-xl text-center text-sm font-bold ${
                  keypadStatus.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                }`}
              >
                {keypadStatus.message}
              </div>
      ...
      {showRoboticaModal && (
        <div
          className="fixed inset-0 z-[70] bg-black/60 flex items-center justify-center p-4 backdrop-blur-md"
          onClick={() => setShowRoboticaModal(false)}
        >
          <div
            className="bg-white rounded-[2rem] p-8 md:p-10 shadow-2xl max-w-2xl w-full flex flex-col gap-8"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 group-hover:text-white/80 mt-1">
                    Soluções avançadas para a indústria e processos produtivos.
                  </p>
                </div>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 group-hover:text-white/80 mt-1">
                    Kits interativos e aprendizado prático para todas as idades.
                  </p>
                </div>
      ...
      {/* Banner de Cookies */}
      {showCookieBanner && (
        <div className="fixed bottom-4 left-4 right-4 md:bottom-8 md:left-1/2 md:right-auto md:-translate-x-1/2 md:w-full md:max-w-6xl z-[100]">
